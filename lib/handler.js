...
Handler.prototype.saveData = async function saveData(token) {
  ...
  this.req.idToken = idToken

  const fetchUser = async () => {
    try {
      return await this.opts.fetchUser(idToken, this.req, this.opts)
    } catch (e) {
      return {}
    }
  }
  console.log("old user: ", this.req[this.opts.sessionName].user)
  const user = this.req[this.opts.sessionName].user === {} ? this.req[this.opts.sessionName].user : await fetchUser()
...
Handler.prototype.updateToken = async function updateToken() {
  ...
  let {
    token
  } = this.req[this.opts.sessionName]
